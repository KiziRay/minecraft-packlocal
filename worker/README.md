...
目前設定以工具 **1.0.0** 為目標。代管 AI 門檻為 **Discord 會員**（Turnstile 已停用）；協定 v3 標頭仍要，舊版會收到 426。
...
- Worker 鎖定模型（`UPSTREAM_MODEL=deepseek-v4-flash`），並預設轉發／補上 `thinking.disabled`，避免被拿去打別的昂貴模型或預設開思考。
