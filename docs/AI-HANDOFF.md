...
- 目前版本必須重新讀取 `src-tauri/Cargo.toml`、`src-tauri/tauri.conf.json`、`package.json`；整理時三處都是 `1.0.2`。
...
- 目前程式版本以三個版本檔為準；整理時確認為 `1.0.3`。根目錄 `AGENTS.md` 已對齊版本敘述與免安裝更新行為，勿再照抄舊的 0.5.1／「禁止自我替換」概述。
