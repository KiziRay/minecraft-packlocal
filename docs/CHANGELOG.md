...
- **正式版共享庫**：覆寫／FTB／任務／Origins／MineMenu／KubeJS 過閘門後貢獻；廢 `__shared_text`（JAR→模組鍵、實例 FTB／kubejs→`pack.*`）；現成繁中進模組庫；長句上限約 8KB。Worker 跨包 votes≥2、包內／`pack.*` 可≥1；取消永久 `conflict`；pending／原文不當 hit。查找：模組庫→JAR 覆寫→本機 TM→AI。
- **略過共享庫查找**：補翻／修復勾選 Force 時只停 lookup（含 overlay），仍可 contribute。
- **代管文案**：額度三句話（個人提供非無限／用盡時代管不可用但共享庫與本機仍可用／可支持或自訂 DeepSeek）；品質提醒；429 引導。
- **診斷多類回報**：`submit_diagnose_report_cmd`；MPU／R2 `reports/v1/`；webhook 只送連結；3 天清理；Discord 會籍；100MB。閃退請先還原。
- **更新檔名**：客戶端只接受 `MCPL-{version}.exe`，拒舊 `-portable.exe`。Worker vars 模板改 1.0.0／MCPL（SHA 等產 exe 後再填）。
- **LICENSE** 維持 PolyForm Noncommercial 1.0.0。

- **24 小時自解檔分享**：`resourcepacks/` 頂層資料夾打成同名 zip 再納入（目錄內不再有子資料夾）；本機 SFX 與下載檔名固定「模組包繁中翻譯自解檔.exe」。成功後按鈕改「複製分享連結」，網址不寫日誌，只留記憶體；換遊戲資料夾或重開工具後需重新分享。落地頁／OG 標題「繁體中文模組包翻譯工具」、副標「讓模組包翻譯不再困難」。公開 URL 改 8 碼短連結（USAGE KV）；長 token 舊檔仍可下載至過期。有 USAGE KV 時每人每日 3 次、同時未過期 2 個，超限 429；未完成 MPU 逾時刪。
- **書本／任務漏翻進遊戲會讀的位置**：Patchouli `landing_text` 與 Citadel／書頁 `.txt` 整檔納入 overlay，並把 `en_us`／`zh_cn` 同步寫 `zh_tw`。JAR 內 Patchouli 改嵌回 `jar-translated` 副本，**不再**把 `work/data` 套到 `minecraft/data/`（無 pack.mcmeta，Forge 不當 datapack）。FTB 任務 `title` 的 JSON text component 只抽／寫 `"text"`（含同一行 `{ id:…, title:… }`），不再把整段 `{"text":"Campaign"}` 當程式碼略過。蒐集盤點新增 `jar_patchouli`／`jar_citadel_books` 桶。
- **書頁路徑／檔名不當顯示文**：`root.txt`、`alligator.json`、相對資源路徑不再送翻；Citadel `assets/*/book/` JSON 的 `parent`／`text`／`linked_page` 當檔名略過。毒譯文不進本機 TM／共享庫。開書 `ResourceLocationException`（`zh_tw/根.txt`）診斷碼 `RESOURCE_PATH_CORRUPTED`。已套用壞 JAR：還原上次套用後用此版重跑。
- **代管 AI 間歇「請先加入 Discord」**：Worker 對已確認會員寫入 `guild_ok` 短時快取（約 15 分）並對 `inGuild≠true` 軟重試一次；客戶端將 `guild_required` 改為可暫態重試（高並行不再整批直接略過）。
- **停止掃尾不卡 96%**：按停止時共享庫掃尾改為限時 3 秒／最多 3000 條／不 flush 舊佇列；成功結尾掃尾亦有條數與時間上限並回報「掃尾結束」。
- **AI 結束後假卡住**：主因是同步上傳社群共享庫（失敗再入隊）長時間無進度；改為寫回後單次貢獻、牆鐘／chunk 上限（逾時暫存佇列）、佇列條數／大小硬上限，並在儲存 TM／寫回／回饋時回報進度。
- **FancyMenu 防毒**：layout 只翻 `label`／`description`／`source`（`text_v2` 正文）等顯示鍵；路徑型 `[source:local]…png` 不翻；`mid-left` 等結構值不進候選；共享庫／本機 TM 命中「token→中文」拒用且不貢獻。舊版弄亂主選單：還原套用／覆蓋備份 `config/fancymenu` 後用新版重翻。歡迎頁大標若為 PNG 仍不翻（產品不處理圖片字）。
- **FTB 同行 description**：`description: ["…"]` 已閉合 array 會抽取／寫回（修標題中、說明仍英）。
- **分享打包白名單**：不含 `jar-translated`／`resourcepacks-extra`／備份／日誌／說明；上傳改 **R2 multipart**（繞過 Worker 100MB body），軟頂約 4GB；NanaZip 子行程隱藏黑窗並回傳結束碼。跨機：翻譯／套用需 WebView2；分享另需 NanaZip。
- **每整合包獨立結果目錄**：預設 `%APPDATA%\modpack-i18n-tool\work\packs\{名稱}-{hash8}\`；舊 `instance-{hash16}` 若已有工作階段會沿用；禁止落到共用 `work\翻譯結果`。
- **忙碌可切窗**：翻譯／字體／診斷進行中仍可切分頁、開說明、縮窗；鎖定再開第二個重任務與改路徑；進度／完成不搶焦點。
- **JAR 顯示文字批次化**：全 JAR 抽出 → 一次 overlay → 只重建有譯文的 JAR；進度改「模組 i/N」；Standard 關閉 `jar_display`（僅 Max）；補充 0 條略過第二次 JAR 重建。
- **共享庫停止掃尾＋防撐大**：停止／結尾只送本輪尚未上傳的 keyhash；Worker `packs` 最多 16（需部署 Worker）。
- **儀表分母本波**：右側命中／略過等分母以本波待譯為準；命中明細預設收合。
- **單一主資源包**：`resourcepacks/` 內 ZIP 不再複製到 `resourcepacks-extra`（語言檔併入主資源包）。
- **佔位符／品質分流**：`is_usable_zh` 未過不再誤入單句嚴格重試；佔位符嚴格重試硬上限 48 句，超量保留英文不重燒。
- **佔位符嚴格重試**：placeholder 不符時快失敗（不反覆重試），失敗保留英文原文。
- **共享庫 contribute 佇列**：本機佇列延遲回饋；Worker glossary 同包可命中（`packCount>=1`），跨包仍需 ≥2（需部署 Worker 才生效雲端）。
- **API 金鑰遮罩**：`########` 僅 UI 顯示；禁止把 `#` 遮罩存成金鑰；儲存／狀態文案标明真金鑰存本機並用於翻譯與測試。
- **掃描語言檔**：寬鬆 JSON 再救 BOM／非法 control／無效 escape；非 en／zh 語系損壞改彙總一則「另略過 N 個非必要語系…」，優先語系仍逐筆列出。
- **自訂 API 金鑰探測**：儲存後自動測、「測試金鑰」按鈕；一鍵翻譯（自訂＋開 AI）開工前嚴格探測，失敗不進長掃描。
- **DeepSeek 官方合規／縮短等待**：自訂與代管請求送 `thinking: {type:"disabled"}`（官方預設開思考會燒 token 至 `finish_reason=length`）；Worker `UPSTREAM_MODEL` 改 `deepseek-v4-flash`（不再用已退役 `deepseek-chat`）；user 明確要求 JSON；`length` 截斷幾乎不重試、立刻拆半；Name／Ui 批 120→48；completion token 依條數提高下限；共享庫／探測補進度心跳；自訂 API 說明內 [platform.deepseek.com](https://platform.deepseek.com) 可點開瀏覽器。
- **資源包名稱**：翻譯前可在「更多選項」自訂；未改建議名或留空時仍由系統產生（`模組包翻譯工具+月日+版本`）；翻譯進行中仍鎖定。
- **自訂 API 文案**：推薦 DeepSeek 便宜划算（[platform.deepseek.com](https://platform.deepseek.com)）；選項小字、服務商、額度提示、說明頁與引導同步。
- **AI 空回應／重試**：空 content 不再誤觸 AIMD 減半並行（僅 429／5xx／逾時等明確 congestion）；空 content／空 JSON 加長退避與 chunk 重試；錯誤訊息帶 `finish_reason`／refusal／choices 鍵名；Name／Ui 大批空回應重排時拆半；Name 批 220→120；content 空時若 `reasoning_content` 可解析為譯文 JSON 則採用。
- **翻譯加速**：代管 AI 並行上限 6→16（DeepSeek 以併發為限；遇 429 仍 AIMD 降速）；加長且跨批不變的 system 前綴以利 Context Caching；JAR 文件複查改為完整度 Max 時在 **AI 之後**補做（Quick／Standard 略過），不再擋主翻譯約 76s。
- AI 管線：批失敗原文寫入進度與錯誤日誌摘要；失敗批可降並行後重送；依 `error.type` 區分預算 429／上游限流／上游 401（不再誤標 Discord）；取消保留已得譯文且步驟 4／extras 不吞成「全部完成」；僅 `login_required` 才開 Discord 重登等待。

## 1.0.0 — 2026-08-16（台北）— 正式版

- **Minecraft＜1.13 硬擋**：偵測或指定 1.12.2 等舊版時前後端拒絕翻譯（年份版 26.x 仍允許）；前端不再把非法版本塞進下拉。
- **錯誤分析系統**：`engine/diagnose/`；`analysis_mode`（`pasted_log`／`pack_dir`）＋`schema_version`；目錄模式讀 crash／latest 並掃描 mods 交叉驗證；無 crash 不高信心定罪；診斷頁路徑欄＋選資料夾；`diagnose_pack_dir`。
- **AI 進度／登入**：進度「批失敗」不再誤標紅字；Discord／`auth_unavailable` 可重載／暫停重登／續跑；逾時保留已成功譯文；中止分類以最新錯誤為準，勿誤標額度用完；額度文案推薦 [platform.deepseek.com](https://platform.deepseek.com)。
- **進度 UI**：步驟短計數／狀態短標籤／數字留在命中格；「停止｜更多選項」同排醒目。
- Phase A～D（0.3.9）納入正式版基線。
- 版本同步：三處＋UI → **1.0.0**。

## 0.3.9 — 2026-08-16（台北）— 上線前：機制欄／亂碼導引／產品固定／AI 管線／進度

上線前完整落地（不含巴哈文案）。**DeceasedCraft** 等已套用舊壞 jar-translated 的實例：請還原後用本版**重建**再套用，勿直接套舊 BAD 隔離夾。

- **Phase A（機制欄）**：Patchouli 改 `AggressiveDisplayFields`；共用 `is_ascii_enum_token`；Aggressive `category` 僅句子；overlay／jar_display 跳過 powers／origins；對齊機制路徑 skip；診斷 `PATCHOULI_TYPE_CORRUPTED`。advancement 維持 DisplayFieldsOnly（不回退）。
- **Phase A2（亂碼）**：字體包置頂／關 Force Unicode；□方框 vs Ã/å/æ 分流；套用警告含 `font/` 的已啟用包；語言檔 UTF-8 smoke（不做 Latin-1 codec 重寫）。診斷碼 `TEXT_MOJIBAKE`。
- **Phase B（產品固定）**：進階解包恒開；一鍵固定 append＋thorough；首頁移除解包／模式／品質 UI；Force 僅補翻／修復勾選；後端保留 Force／Skip。
- **Phase C（AI 管線）**：Worker 白名單 `response_format`／`max_tokens`；固定 system＋純 JSON user；json-mode；provider 降級；usage／cache 指標；分軌組批；暖機 AIMD；只重送未解決；術語一致化；TM 防毒。佔位符負向快取沿用既有（不重做第二套）。
- **Phase D（進度＋防卡死）**：結構化 `ProgressPayload`；加權％；節流；前端 stage／coalesce／AI 面板／狀態徽章。
- 版本同步：三處＋UI → **0.3.9**。

## 0.3.8 — 2026-08-16（台北）— 補充漏翻／自解分享／覆蓋升級

- **緊急修正（建立世界崩潰）**：advancement 只翻顯示欄位（`ADVANCEMENT_CORRUPTED`）。
- **五步流程**／分享自解 exe／覆蓋與效能／佔位符負向快取等（見本版細節）。
- 版本同步：三處＋UI → **0.3.8**。

## 0.3.6 — 2026-08-16（台北）— 縮放全版面

- **版面**：非 100% 縮放時取消 `.app` 的 `margin: auto` 置中；改 `100vw`／`100dvh` 補償＋左上錨點，左右不再留灰邊；高度補償保留，footer 仍不被裁切。
- 版本同步：三處＋UI → **0.3.6**。

## 0.3.5 — 2026-08-16（台北）— Footer 縮放／任務補充／忙碌 UI／ETA／日誌／開發偵測

- **Footer**：`.app` 以 `calc(... / --ui-scale)` 補償 zoom，縮放 >100% 不再裁掉頁尾。
- **任務補充**：三步說明＋勾選「已完成遊戲內步驟」後才可「③ 重新翻譯任務文字」。
- **忙碌 UI**：翻譯中不再收合主區／最小化；控件鎖定，**停止**仍可按。
- **預估剩餘**：百分比前進只准變短；卡住改倒數，不再隨總耗時一直變長。
- **日誌**：AI 等待進度去重；後端約 10 秒一推。「報告」覆寫開啟 `執行日誌.txt`（有上限），不再開 `翻譯工作階段.json`。
- **開發進度偵測**：工作根寫入 `開發進度偵測.txt`（每次覆寫、≤512KB），含階段計時與結束排行，供優化分析。
- 版本同步：三處＋UI → **0.3.5**。

## 0.3.4 — 2026-08-16（台北）— Footer／縮放提示／去背 Icon／新手引導

- **Footer**：任何介面縮放百分比下都顯示；矮視窗改為壓扁樣式，不再 `display: none`（修正 `.app zoom` 觸發 `max-height` 隱藏）。
- **縮放鎖定提示**：自動縮放開啟時，Ctrl＋滾輪／方向鍵在游標旁顯示「介面縮放已鎖定」（對齊工具箱）。
- **Icon**：統一透明底方塊＋書本 LOGO，重產 `app-logo.png` 與 `packlocal-v3` PNG／多尺寸 ICO；移除未使用的 empty-state／service-icons。
- **新手引導**：首次啟動 spotlight 五步（分頁→資料夾→AI→開始翻譯→⋯）；`localStorage` 旗標；⋯ 選單可重播。
- 版本同步：三處＋UI → **0.3.4**。

## 0.3.3 — 2026-08-15（台北）— 無框標題列／自動縮放／分享落地／共享庫閘門

- **視窗外框**：`decorations: false`＋自訂 winbar（拖曳／八向縮放／最小化／最大化／關閉）；新 LOGO／icon；介面去 ZF／zeitfrei／packlocal 字樣（Discord／珍奶連結保留）。
- **自動縮放**：依 `screen.availWidth` 階梯調整，預設開；⋯ 選單可關；手動 Ctrl＋滾輪在自動開啟時改提示。
- **⋯ 選單**：抬高堆疊，修正 125% 縮放下「瀏覽…」穿透與 checkbox 空橫條。
- **分享**：僅本機確有可分享檔時顯示按鈕；落地頁補 OG／Discord／珍奶推廣與 `/share-og.png`。
- **共享庫**：contribute 前佔位符＋可用繁中閘門；回報 accepted／衝突；共享術語 lookup 可觀測；免 AI 比例寫入補譯 note；Force 模式標明較貴。
- **文案**：AI 區塊改「AI 輔助翻譯」「啟用 AI 幫你翻譯」；命中明細預設展開。
- 版本同步：三處＋UI → **0.3.3**（Worker 下載資產等建置後再 deploy）。

## 0.3.2 — 2026-08-15（台北）— 翻譯搜尋與品質完整修正

- **品質閘門**：假 `zh_tw`（仍英／中英混雜）不再鎖死 pending；優先可用 `zh_cn`→s2tw；Skip90／補翻／參考合併皆看可用率。
- **片語／AI**：片語字界套用；短詞 hints 跳過；整句規則強化；拒收混雜寫入 TM／共享庫。
- **覆寫大補**：Patchouli `en_us`→同步 `zh_tw`；StarterKit 純文字；firstJoin／deathbackup；ArmorSets `armorSetName`；FancyMenu 無引號 `description`＋`.local`。
- **MineMenu**：翻譯 `title` 並 `\uXXXX` 寫回（不再只 escape 英文）。
- **搜尋誠實**：`units=0` 標「僅盤點」；覆蓋說明標明語言檔層級％不含 overlay／硬編碼。
- **共享庫可觀測**：空庫／失敗／命中／貢獻計數寫入 note；`/health` 加 `translationsBound`。
- **UI**：有輸出／工作階段即可「分享翻譯結果」；關閉縮背景移到 ⋯（外觀旁）。
- **誠實範圍**：Essential／MIDI／Drop Rate 等硬編碼寫入覆蓋說明（不改 class）。
- 版本同步：三處＋UI／Worker → **0.3.2**。

## 0.3.1 — 2026-08-15（台北）— FTB 結構守衛／診斷／補缺口／UI

- **搶救**：CTE2 等實例若 type／shape 已被翻壞，可用 `scripts/rescue_ftb_types.py` 對照原整合包救回；`act_0.snbt` 等應恢復 `type: "checkmark"`。
- **FTB**：`ftbquests.rs` 改 DISPLAY_KEYS 白名單（title／description／subtitle）＋ STRUCT_KEYS 黑名單；欄位感知 extract／rewrite，不再翻 type／shape／auto。
- **二次補翻**：lang `pending=0` 略過 AI fill；FTB／額外來源只送「仍為英文」的顯示字串，已是繁中則跳過（優先讀工作目錄副本）。
- **診斷**：偵測 `ResourceLocationException`＋`ftbquests:`／非 ASCII path → `FTB_TYPE_CORRUPTED`，提示還原備份或重跑新版工具。
- **AI**：代管並行批次 4→6（小幅提速）；TM／glossary 命中路徑不變。
- **UI**：步驟不再因過程「套用」文案閃爍 done；busy 鎖診斷分頁；合併「開啟輸出／報告」；翻譯開始依偏好最小化或收合主區；單實例鎖（`tauri-plugin-single-instance`）；首頁註記自動套用／進階解包；贊助區塊強化。
- 版本同步：三處＋UI → **0.3.1**。
...
- **雙擊蓋層**：清掉空白處選取範圍；不再用過猛的 dblclick `preventDefault` 攔截互動元件。
- **支持開發**：移除主區漂浮重複鈕，只留 footer。
- **條件顯示**：未通過 `validate_instance` 前隱藏 AI、開始翻譯、右欄步驟／日誌；通過後再展開並重排單欄→雙欄。
- 版本同步：三處＋UI／說明／Worker → **0.2.3**。

## 0.2.2 — 2026-08-14（台北）— 全版面＋固定完整挑戰＋實例門檻

- **全版面**：工作區吃滿可用寬高（流體 padding），去掉中央固定窄卡＋大片黑邊；縮小仍可重排。
- **雙擊空白**：對 shell 設 `user-select: none`（輸入區可選），並擋有害 dblclick／dragstart，避免 WebView 選取卡住。
- **完整度**：UI 移除快速／標準三卡與右欄「完整度」區塊；主路徑固定 `max`（完整挑戰），進度併入步驟列。
- **AI／驗證上主頁**：代管／自訂與驗證摺疊改在翻譯主頁；`ai_status` 不再把 Turnstile health 誤標成 Discord「暫時無法」。
- **實例門檻**：新增 `validate_instance`（mods＋模組檔＋實例特徵）；前端禁用開始、後端 one_click 再擋一次。
- **預設**：不自動覆寫偏好／不預填參考包路徑；參考文案改通用（不再寫死 CTE2）。
- 版本同步：三處＋UI／說明／Worker → **0.2.2**。
